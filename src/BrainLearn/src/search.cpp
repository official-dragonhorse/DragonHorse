...
void Search::Worker::iterative_deepening() {
    ...
    if (!mainThread && bool(options["MCTS"]) && multiPV == 1 && !maybeDraw && possibleMCTSByValue
        && ((this->thread_idx) <= (size_t) (mctsThreads))
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && !disableNMAndPC  //Kelly
        && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 17379 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 21 * depth + 329 && !excludedMove
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    // from Sugar
    if (openingVariety && bestValue + (openingVariety * NormalizeToPawnValue / 100) >= 0
        && pos.count<PAWN>() > 12)
